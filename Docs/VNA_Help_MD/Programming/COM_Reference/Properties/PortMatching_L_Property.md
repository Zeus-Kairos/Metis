...
|  Sets and returns the value for the 'L' (Inductance) circuit element for a
port matching circuit model to simulate on port 'n'. The port matching circuit
model is selected using
[PortMatchingCktModel](PortMatchingCktModel_Property.md). You can specify L,
L1, or L2 based on the selected port matching circuit model. Setting a value
