...
def test_unique_constraints():
    ...
    try:
        ...
        # Cleanup users created during this test
            ...
            try:
                with memory_manager.connection_pool.connection() as conn:
                    with conn.cursor() as cur:
                        ...
                        user = cur.fetchone()
                        if user:
                            memory_manager.delete_user(user[0])
                            print(f"Cleanup: Deleted user with email {email}")
...
def test_knowledgebase_name_unique_per_user():
    ...
    try:
        ...
        # Cleanup - delete the users (which will cascade delete their knowledgebases)
        print("\nCleaning up test data...")
        
        for user_email in [email, email2]:
            try:
                with memory_manager.connection_pool.connection() as conn:
                    with conn.cursor() as cur:
                        ...
                        if user:
                            memory_manager.delete_user(user[0])
                            print(f"Cleanup: Deleted user with email {user_email}")
            except Exception as e:
                print(f"Cleanup error for {user_email}: {e}")
...
if __name__ == "__main__":
    ...
    print("\nAll unique constraint tests completed!")