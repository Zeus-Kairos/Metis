...
class Qwen3VLEmbeddings(Embeddings):
    def __init__(self, model_name: str = "Qwen/Qwen3-VL-Embedding-2B"):
        ...
        self.embedding_model = Qwen3VLEmbedder(model_name_or_path=self.model_name)

    def embed_query(self, text: str) -> List[float]:
        """
        Get the embedding for a single query text.
        ...
        """
        queries = [{"text": text}]
        embeddings = self.embedding_model.process(queries)
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        """
        queries = [{"text": text} for text in texts]
        embeddings = self.embedding_model.process(queries)
